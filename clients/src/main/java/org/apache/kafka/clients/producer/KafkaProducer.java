...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        ProducerConfig config = new ProducerConfig(ProducerConfig.appendSerializerToConfig(configs, keySerializer,
                valueSerializer)); // 配置的HashMap格式转成ProducerConfig 在此期间如果构造方法指定了key和value的序列化器 也放到配置里面 如果没指定就不管
        try {
            Map<String, Object> userProvidedConfigs = config.originals(); // 用户自定义配置
