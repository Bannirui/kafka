 ...
 */
case class EndPoint(host: String, port: Int, listenerName: ListenerName, securityProtocol: SecurityProtocol) { // host：Broker主机名 port：Broker端口号 listenerName：监听器名字 securityProtocol：监听器使用的安全协议 kafka支持4种安全协议为为PLAINTEXT、SSL、SSL_PLAINTEXT、SASL_SSL
  def connectionString: String = { // 构造完整的监听器连接字符串 格式：监听器名称://主机名:端口
    val hostport =
      if (host == null)
        ":"+port
      else
  ...
  def toJava: JEndpoint = { // clients工程下又一个java版本的EndPoint类供clients端代码使用 此方法构造java版本的EndPoint类实例
