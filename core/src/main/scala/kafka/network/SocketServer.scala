...
class SocketServer(val config: KafkaConfig,
  ...
  extends Logging with KafkaMetricsGroup with BrokerReconfigurable { // 封装了网络通信逻辑

  private val maxQueuedRequests = config.queuedMaxRequests // SocketServer实现了BrokerReconfigurable trait表明SocketServer的一些参数配置是允许动态修改的 即在Broker不停机的情况下进行修改 SocketServer的氢气队列长度 由Broker端参数queued.max.requests值而定 默认值是500
  ...
  private[network] val dataPlaneAcceptors = new ConcurrentHashMap[EndPoint, Acceptor]() // 处理数据类请求的Acceptor线程池 每套监听器对应一个Acceptor线程
  val dataPlaneRequestChannel = new RequestChannel(maxQueuedRequests, DataPlaneMetricPrefix, time, allowDisabledApis) // 处理数据类请求你专属的RequestChannel对象
  // control-plane
  private var controlPlaneProcessorOpt : Option[Processor] = None // 用于处理控制类请求的Processor线程
  private[network] var controlPlaneAcceptorOpt : Option[Acceptor] = None
  val controlPlaneRequestChannelOpt: Option[RequestChannel] = config.controlPlaneListenerName.map(_ =>
    new RequestChannel(20, ControlPlaneMetricPrefix, time, allowDisabledApis)) // 处理控制类请求专属的RequestChannel对象
  ...
  def startProcessingRequests(authorizerFutures: Map[Endpoint, CompletableFuture[Void]] = Map.empty): Unit = {
    ...
    this.synchronized {
      if (!startedProcessingRequests) {
        startControlPlaneProcessorAndAcceptor(authorizerFutures) // 启动处理控制类请求的Processor和Acceptor线程
        startDataPlaneProcessorsAndAcceptors(authorizerFutures) // 启动处理数据类请求的Processor和Acceptor线程
  ...
  private def startDataPlaneProcessorsAndAcceptors(authorizerFutures: Map[Endpoint, CompletableFuture[Void]]): Unit = {
    val interBrokerListener = dataPlaneAcceptors.asScala.keySet
      ...
      .getOrElse(throw new IllegalStateException(s"Inter-broker listener ${config.interBrokerListenerName} not found, endpoints=${dataPlaneAcceptors.keySet}")) // 获取Broker间通讯所用的所有的监听器 默认是PLAINTEXT
    val orderedAcceptors = List(dataPlaneAcceptors.get(interBrokerListener)) ++
      dataPlaneAcceptors.asScala.filter { case (k, _) => k != interBrokerListener }.values
    orderedAcceptors.foreach { acceptor =>
      val endpoint = acceptor.endPoint
      startAcceptorAndProcessors(DataPlaneThreadPrefix, endpoint, acceptor, authorizerFutures) // 启动Processor和Acceptor线程
    }
  }
  ...
  private def createDataPlaneAcceptorsAndProcessors(dataProcessorsPerListener: Int,
    ...
    endpoints.foreach { endpoint => // 遍历监听器集合
      connectionQuotas.addListener(config, endpoint.listenerName) // 将监听器纳入到连接配额管理之下
      val dataPlaneAcceptor = createAcceptor(endpoint, DataPlaneMetricPrefix) // 为监听器创建对应的Acceptor线程
      addDataPlaneProcessors(dataPlaneAcceptor, endpoint, dataProcessorsPerListener) // 为监听器创建多个Processor线程 具体数据由num.network.threads决定
      dataPlaneAcceptors.put(endpoint, dataPlaneAcceptor) // 将<监听器, Acceptor线程>对保存起来统一管理
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = {
    endpointOpt.foreach { endpoint =>
      connectionQuotas.addListener(config, endpoint.listenerName) // 将监听器纳入到连接配额管理之下
      val controlPlaneAcceptor = createAcceptor(endpoint, ControlPlaneMetricPrefix) // 为监听器创建对应的Acceptor线程
      val controlPlaneProcessor = newProcessor(nextProcessorId, controlPlaneRequestChannelOpt.get,
        connectionQuotas, endpoint.listenerName, endpoint.securityProtocol, memoryPool, isPrivilegedListener = true) // 为监听器创建对应的Processor线程
      controlPlaneAcceptorOpt = Some(controlPlaneAcceptor)
      controlPlaneProcessorOpt = Some(controlPlaneProcessor)
      val listenerProcessors = new ArrayBuffer[Processor]()
      listenerProcessors += controlPlaneProcessor
      controlPlaneRequestChannelOpt.foreach(_.addProcessor(controlPlaneProcessor)) // 将Processor线程添加到控制类请求专属RequestChannel中 即添加到RequestChannel实例保存的Processor线程池中
      nextProcessorId += 1
      controlPlaneAcceptor.addProcessors(listenerProcessors, ControlPlaneThreadPrefix) // 把Processor对象也添加到Acceptor线程管理的Processor线程池中
...
object SocketServer { // 定义了与SocketServer相关的公共常量
 ...
 */
private[kafka] abstract class AbstractServerThread(connectionQuotas: ConnectionQuotas) extends Runnable with Logging { // SocketServer各类线程抽象父类 是Acceptor线程和Processor线程的抽象基类 定义了这两个线程的公有方法
 ...
 */
private[kafka] class Acceptor(val endPoint: EndPoint, // kafka Broker连接信息
                              val sendBufferSize: Int, // 设置的是SocketOptions的SO_SNDBUF 即用于设置出站(Outbound)网络IO的底层缓冲区大小 该值默认是Broker端参数socket.send.buffer.bytes的值100KB
                              val recvBufferSize: Int, // SocketOptions的SO_RCVBUF 即设置入站(Inbound)网络IO的底层缓冲区大小 该值默认是Broker端参数socket.receive.buffer.bytes的值即100KB
                              brokerId: Int,
                              connectionQuotas: ConnectionQuotas,
                              metricPrefix: String,
                              time: Time) extends AbstractServerThread(connectionQuotas) with KafkaMetricsGroup { // 接收和创建外部TCP连接的线程 每个SocketServer实例只会创建一个Acceptor线程 位移目的就是创建连接 并将接收到的Request传递给下游的Processor线程处理 经典的Reactor模式有个Dispatcher角色 接收外部请求并分发给下面的实际处理线程 Acceptor线程就扮演着Dispatcher的角色

  private val nioSelector = NSelector.open() // 创建底层的NIO Selector对象 Selector对象负责执行底层实际IO操作 如监听连接创建请求、读写请求等
  val serverChannel = openServerSocket(endPoint.host, endPoint.port) // Broker端创建对应的ServerSocketChannel实例 后续把该Channel向上一步的Selector对象注册
  private val processors = new ArrayBuffer[Processor]() // 创建Processor线程池 实际上就是Processor线程数组
  ...
  private def startProcessors(processors: Seq[Processor], processorThreadPrefix: String): Unit = synchronized {
    processors.foreach { processor =>
      KafkaThread.nonDaemon(
        ...
        processor
      ).start() // 依次创建并启动Processor线程 线程命名规范：processor线程前缀-kafka-network-thread-broker序号-监听器名称-安全协议-Processor序号
    }
  }
  ...
  private[network] def removeProcessors(removeCount: Int, requestChannel: RequestChannel): Unit = synchronized {
    ...
    val toRemove = processors.takeRight(removeCount) // 获取Processor线程池中最后removeCount个线程
    processors.remove(processors.size - removeCount, removeCount) // 移除最后removeCount个线程
    toRemove.foreach(_.initiateShutdown()) // 关闭最后removeCount个线程
    toRemove.foreach(_.awaitShutdown())
    toRemove.foreach(processor => requestChannel.removeProcessor(processor.id)) // 在RequestChannel中移除这些Processor
  }
...
private[kafka] object Processor { // 定义与Processor线程相关的公共常量
...
private[kafka] class Processor(val id: Int,
                               ...
                               allowDisabledApis: Boolean = false) extends AbstractServerThread(connectionQuotas) with KafkaMetricsGroup { // Processor线程类 这是处理单个TCP连接上所有请求的线程 每个SocketServer实例默认创建若干个(num.network.threads)Processor线程 Processor线程负责将接收到的Request添加到RequestChannel的Request队列上 同时还负责将Response返回给Request发送方
  ...
  private val newConnections = new ArrayBlockingQueue[SocketChannel](connectionQueueSize) // 保存要创建的新连接信息 就是SocketChannel对象 默认上限是20的队列 每当Processor线程接收新的连接请求时 都会将对应的SocketChannel放入这个队列 后面在创建连接时就从该队列取出SocketChannel 然后注册新的连接
  private val inflightResponses = mutable.Map[String, RequestChannel.Response]() // 临时Response队列 当Processor线程将Response返还给Request发送方之后 还要将Response放入这个临时队列 需要这个队列的原因是因为有些Response回调逻辑要在Response被发送回发送方之后才能执行 因此需要暂存在一个临时队列里面
  private val responseQueue = new LinkedBlockingDeque[RequestChannel.Response]() // 这是Response队列而不是Request队列 每个Response线程都会维护自己的Response队列
    ...
    try {
      while (isRunning) {
        try {
          ...
          poll() // 执行NIO poll 获取对应SocketChannel上准备就绪的IO操作
          processCompletedReceives() // 将接收到的Request放入到Request队列
  ...
  private def processNewResponses(): Unit = { // 负责发送Response给Request发送方 并且将Response放入临时Response队列
  ...
  private def poll(): Unit = {
    ...
    try selector.poll(pollTimeout) // 本质上是调用java NIO Selector的select()方法去执行那些准备就绪的IO操作
    catch {
  ...
  private def processCompletedReceives(): Unit = {
    selector.completedReceives.forEach { receive => // 遍历锁欧已接收的Request
      try {
        openOrClosingChannel(receive.source) match { // 保证对应连接通道已经建立
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) { // 如果认证会话已经过期就关闭连接
                ...
                if (req != null) {
                  ...
                  requestChannel.sendRequest(req) // 核心：将Request添加到Request队列
  ...
  private def processCompletedSends(): Unit = { // 处理Response的回调逻辑 Response需要被发送之后才能执行对应的回调逻辑
    selector.completedSends.forEach { send => // 遍历底层SocketChannel已发送的Response
      try {
        ...
        response.onComplete.foreach(onComplete => onComplete(send)) // 执行回调逻辑
  ...
  private def processDisconnected(): Unit = { // 处理已断开连接的
    selector.disconnected.keySet.forEach { connectionId => // 遍历底层SocketChannel的那些已断开的连接
      try {
        val remoteHost = ConnectionId.fromString(connectionId).getOrElse {
          throw new IllegalStateException(s"connectionId has unexpected format: $connectionId")
        }.remoteHost // 获取断开连接的远端主机名信息
        inflightResponses.remove(connectionId).foreach(updateRequestMetrics) // 将该连接从inflightResponses中移除 同时更新一些监控指标
        // the channel has been closed by the selector but the quotas still need to be updated
        connectionQuotas.dec(listenerName, InetAddress.getByName(remoteHost)) // 更新配额数据
      } catch {
  ...
  private def closeExcessConnections(): Unit = { // 关闭超限连接
    if (connectionQuotas.maxConnectionsExceeded(listenerName)) { // 如果配额超限了
      val channel = selector.lowestPriorityChannel() // 找出优先关闭的那个连接
      if (channel != null)
   ...
   */
  private def configureNewConnections(): Unit = { // 负责处理新连接请求
    var connectionsProcessed = 0 // 当前已经配置的连接数计数器
    while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {
      val channel = newConnections.poll()
      try {
        debug(s"Processor $id listening to new connection from ${channel.socket.getRemoteSocketAddress}")
        selector.register(connectionId(channel.socket), channel) // 用给定的Selection注册该Channel 底层就是调用java NIO的SocketChannel.register(selector, SelectionKey.OP_READ)
...
class ConnectionQuotas(config: KafkaConfig, time: Time, metrics: Metrics) extends Logging with AutoCloseable { // 控制连接配额的类 能够设置单个IP创建Broker连接的最大数量以及单个Broker能够允许的最大连接数
...
class TooManyConnectionsException(val ip: InetAddress, val count: Int) extends KafkaException(s"Too many connections from $ip (maximum = $count)") // 标识连接数过多的异常类
