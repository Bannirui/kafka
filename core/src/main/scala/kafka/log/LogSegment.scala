 ...
 */
@nonthreadsafe
class LogSegment private[log] (val log: FileRecords, // kafka消息的对象
                               val lazyOffsetIndex: LazyIndex[OffsetIndex], // 延迟初始化降低初始化成本
                               val lazyTimeIndex: LazyIndex[TimeIndex], // 延迟初始化降低初始化成本
                               val txnIndex: TransactionIndex,
                               val baseOffset: Long, // 每个日志段对象保存自己的起始位移baseOffset 每个LogSegment对象实例被创建 它的起始位移就是固定的不能被改变
                               val indexIntervalBytes: Int, // Broker端参数log.index.interval.bytes控制日志段对象新增索引的频率 默认情况下日志段对象至少新写入4K的消息才会新增一条索引项 也就是说在未来某个时刻可能同时创建多个日志段对象导致磁盘IO压力增加
                               val rollJitterMs: Long, // 通过rollJitterMs值干扰使日志段创建时岔开一小段时间减轻IO瓶颈
                               val time: Time) extends Logging { // Time用于统计计时
   ...
   * @param shallowOffsetOfMaxTimestamp The offset of the message that has the largest timestamp in the messages to append. // 最大时间戳对应的消息位移值
  ...
  def append(largestOffset: Long,
             ...
             records: MemoryRecords): Unit = { // 写入消息
    if (records.sizeInBytes > 0) {
      trace(s"Inserting ${records.sizeInBytes} bytes at end offset $largestOffset at position ${log.sizeInBytes} " +
            s"with largest timestamp $largestTimestamp at shallow offset $shallowOffsetOfMaxTimestamp")
      val physicalPosition = log.sizeInBytes() // 判断日志段是否为空
      if (physicalPosition == 0)
        rollingBasedTimestamp = Some(largestTimestamp) // 如果日志段为空 记录要写入消息集合中的最大时间戳 作为后面新增日志段倒计时的依据

      ensureOffsetInRange(largestOffset) // 确保输入参数的最大位移值是合法的: 0<= (largestOffset-baseOffset) <= int.MAX_VALUE

      // append the messages
      val appendedBytes = log.append(records) // FileRecords#append()执行真正的写入: 内存中的消息对象写入到操作系统的页缓存中
      trace(s"Appended $appendedBytes to ${log.file} at end offset $largestOffset")
      // Update the in memory max timestamp and corresponding offset.
      if (largestTimestamp > maxTimestampSoFar) { // 更新日志段的最大时间戳以及最大时间戳对应的消息的位移值
        maxTimestampSoFar = largestTimestamp // Broker端提供的定期删除日志功能就是依据这个属性值
        offsetOfMaxTimestampSoFar = shallowOffsetOfMaxTimestamp
      }
      // append an entry to the index (if needed) // 更新索引项和写入字节数 日志段每写入4K数据就要写入一个索引项 当已写入字节超过4K就调用索引对象的append()方法新增索引项 同时清空已写入字节 以备下次重新累积计算
   ...
   *
   * @param startOffset A lower bound on the first offset to include in the message set we read // 要读取的第一条消息的位移值
   ...
   * @param minOneMessage If this is true, the first message will be returned even if it exceeds `maxSize` (if one exists) // 是否允许在消息体过大时至少返回第一条消息 当该参数为true时即使出现消息体字节数超过了maxSize的情形read()方法至少返回第一条消息 引入这个参数是为了确保不出现消费饿死的情况
   *
  ...
  def read(startOffset: Long,
           ...
           minOneMessage: Boolean = false): FetchDataInfo = { // 读取日志段
    if (maxSize < 0)
      throw new IllegalArgumentException(s"Invalid max size $maxSize for log read from segment $log")

    val startOffsetAndSize = translateOffset(startOffset) // 定位要读取的起始文件位置 入参startOffset仅仅是位移值 kafka需要根据索引信息找到对应的物理文件位置才能开始读取消息
    ...
    val fetchSize: Int = min((maxPosition - startPosition).toInt, adjustedMaxSize) // 能够读取的总字节数

    FetchDataInfo(offsetMetadata, log.slice(startPosition, fetchSize),
      firstEntryIncomplete = adjustedMaxSize < startOffsetAndSize.size) // FileRecords#slice()从指定位置读取指定大小的消息
  }
   ...
   */
  @nonthreadsafe
  def recover(producerStateManager: ProducerStateManager, leaderEpochCache: Option[LeaderEpochFileCache] = None): Int = { // 恢复日志段 Broker在启动时会从磁盘上加载所有的日志段信息到内存中 并且创建LogSegment对象实例
    offsetIndex.reset() // 索引对象的reset()方法清空索引文件: 位移索引文件
    timeIndex.reset() // 索引对象的reset()方法清空索引文件: 时间戳索引文件
    txnIndex.reset() // 索引对象的reset()方法清空索引文件: 已终止事务索引文件
    var validBytes = 0
    var lastIndexEntry = 0
    maxTimestampSoFar = RecordBatch.NO_TIMESTAMP
    try { // 遍历日志段中所有的消息集合或消息批次 对于读到的每个消息集合 日志段必须确保它们是合法的: 1,该消息集合中的消息必须符合kafka中定义的二进制格式 2,该集合中最后一条消息的位移值不能越界，它与日志段起始位移值的差值必须是一个正整数值
      for (batch <- log.batches.asScala) {
        batch.ensureValid()
        ensureOffsetInRange(batch.lastOffset)

        // The max timestamp is exposed at the batch level, so no need to iterate the records
        if (batch.maxTimestamp > maxTimestampSoFar) { // 遍历过程中更新最大时间戳以及最大时间戳对应的消息位移值 这两个值用于后续构建索引项
        ...
        validBytes += batch.sizeInBytes() // 累加已读取字节数
    ...
    val truncated = log.sizeInBytes - validBytes // 日志段当前总字节数-累加已读取字节数
    if (truncated > 0)
