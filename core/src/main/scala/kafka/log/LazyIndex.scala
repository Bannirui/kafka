...
/**
  ...
  */
@threadsafe
class LazyIndex[T <: AbstractIndex] private (@volatile private var indexWrapper: IndexWrapper, loadIndex: File => T) { // 定义了AbstractIndex上的一个包装类 实现索引项延迟加载 这个类的设计主要是为了提高性能
