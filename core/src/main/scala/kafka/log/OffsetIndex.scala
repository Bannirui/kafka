...
class OffsetIndex(_file: File, baseOffset: Long, maxIndexSize: Int = -1, writable: Boolean = true)
    extends AbstractIndex(_file, baseOffset, maxIndexSize, writable) { // 定义位移索引 保存了 位移值-文件磁盘物理位置
  ...
  def lookup(targetOffset: Long): OffsetPosition = {
    maybeLock(lock) {
      ...
      val slot = largestLowerBoundSlotFor(idx, targetOffset, IndexSearchType.KEY) // largestLowerBoundSlotFor这个方法底层使用了改进版的bs算法寻找对应的槽
      if(slot == -1) // 如果没找到 返回一个空的位置 即物理文件位置从0开始 表示从头读日志文件 否则返回slot槽对应的索引项
  ...
  def append(offset: Long, position: Int): Unit = {
    inLock(lock) {
      require(!isFull, "Attempt to append to a full index (size = " + _entries + ").") // 判断索引文件未写满
      if (_entries == 0 || offset > _lastOffset) { // 必须满足以下2个条件之一才允许写入索引项 1，当前索引文件为空 2，要写入的位移大于当前所有已写入的索引项的位移(kafka中索引项中的位移值必须是单调增加的)
        trace(s"Adding index entry $offset => $position to ${file.getAbsolutePath}")
        mmap.putInt(relativeOffset(offset)) // 向mmap中写入相对位移值
        mmap.putInt(position) // 向mmap中写入物理位置信息
        _entries += 1 // 更新其他元数据统计信息 如当前索引项计数器_entries和当前索引项最新位移值_lastOffset
        _lastOffset = offset
        require(_entries * entrySize == mmap.position(), s"$entries entries but file position in index is ${mmap.position()}.") // 执行校验 写入的索引项格式必须符合要求 即索引项个数*单个索引项占用字节数匹配当前文件物理大小 否则说明文件已经损坏
      } else {
        throw new InvalidOffsetException(s"Attempt to append an offset ($offset) to position $entries no larger than" +
          s" the last offset appended (${_lastOffset}) to ${file.getAbsolutePath}.") // 如果不能执行写入索引项操作就抛出异常
      }
    }
  }
   ...
   */
  private def truncateToEntries(entries: Int): Unit = { // 截断操作 将索引文件内容直接裁剪掉一部分
    inLock(lock) {
      _entries = entries
