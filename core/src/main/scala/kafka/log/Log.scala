...
object LogAppendInfo { // 创建特定的LogAppendInfo对象实例
...
case class LogAppendInfo(var firstOffset: Option[Long],
                         ...
                         var recordConversionStats: RecordConversionStats, // 消息统计转换类 里面记录了执行了格式转换的消息
                         sourceCodec: CompressionCodec, // 消息集合中消息使用的压缩器类型 比如是Snappy还是LZ4
                         targetCodec: CompressionCodec, // 写入消息时使用的压缩器类型
                         shallowCount: Int, // 消息批次数 每个消息批次下可能包含多条消息
                         validBytes: Int, // 写入消息总字节数
                         offsetsMonotonic: Boolean, // 消息位移值是否是顺序增加的
                         lastOffsetOfFirstBatch: Long, // 首个消息批次中最后一条消息的位移值
                         recordErrors: Seq[RecordError] = List(), // 写入消息时出现的异常列表
                         errorMessage: String = null, // 错误码
                         leaderHwChange: LeaderHwChange = LeaderHwChange.None) { // 保存了一组待写入消息的各种元数据信息: 消息中第一条消息的位移值、最后一条消息的位移值、这组消息中最大的消息时间戳
  /**
...
case class LogOffsetSnapshot(logStartOffset: Long,
                             ...
                             lastStableOffset: LogOffsetMetadata) // 封装分区所有位移数据的容器类
...
case class LogReadInfo(fetchedData: FetchDataInfo,
                       ...
                       logStartOffset: Long,
                       logEndOffset: Long,
                       lastStableOffset: Long) // 封装读取日志返回的数据及其元数据
 ...
 */
case class CompletedTxn(producerId: Long, firstOffset: Long, lastOffset: Long, isAborted: Boolean) { // 记录已完成事务的元数据 主要用于构建事务索引
...
case class RollParams(maxSegmentMs: Long,
                      ...
                      messagesSize: Int,
                      now: Long) // 定义用于控制日志段是否切分Roll的数据结构

object RollParams { // 伴生类工厂方法
 ...
 */
@threadsafe
class Log(@volatile private var _dir: File, // 这个日志所在的文件夹路径 也就是主题分区的路径
  ...
  @volatile private var nextOffsetMetadata: LogOffsetMetadata = _ // 封装了下一条待插入消息的位移值 就是LEO
   ...
   */
  @volatile private var highWatermarkMetadata: LogOffsetMetadata = LogOffsetMetadata(logStartOffset) // 分区日志高水位值 1,并发场景下 高水位值通过jvm monitor锁同步 volatile关键字保证线程可见性 2,高水位值的初始值是Log Start Offset值

  /* the actual segments of the log */
  private val segments: ConcurrentNavigableMap[java.lang.Long, LogSegment] = new ConcurrentSkipListMap[java.lang.Long, LogSegment] // 保存了分区日志下所有的日志段信息 Map数据结构保存 key=日志段的起始位移值 value=日志段对象本身 ConcurrentSkipListMap线程安全并且key是可排序的

  // Visible for testing
  @volatile var leaderEpochCache: Option[LeaderEpochFileCache] = None // 用来判断出现Failure时是否执行日志截断操作

  locally { // 相当于java代码中的static关键字
    // create the log directory if it doesn't exist
    Files.createDirectories(dir.toPath) // 创建分区日志路径 保存Log对象磁盘文件

    initializeLeaderEpochCache() // 初始化Leader Epoch Cache

    val nextOffset = loadSegments() // 加载所有日志段对象 并返回该Log对象下一条消息的位移值

    /* Calculate the offset of the next message */
    nextOffsetMetadata = LogOffsetMetadata(nextOffset, activeSegment.baseOffset, activeSegment.size) // 初始化LEO元数据对象 LEO值为为上面获取的位移值 起始位移值是ActiveSegment的起始位移值 日志段大小是ActiveSegment的大小

    leaderEpochCache.foreach(_.truncateFromEnd(nextOffsetMetadata.messageOffset)) // 更新Leader Epoch缓存 去除LEO值之上的所有无效缓存项
  ...
  def highWatermark: Long = highWatermarkMetadata.messageOffset // getter读取高水位位移值
   ...
   */
  def updateHighWatermark(hw: Long): Long = { // 使用场景：Follower副本从Leader副本获取到消息后更新高水位值 一旦拿到消息就必须更新高水位值
    val newHighWatermark = if (hw < logStartOffset) // LogStartOffset <= 新的高水位值 <= LEO
      logStartOffset
    else if (hw > logEndOffset)
      logEndOffset
    else
      hw
    updateHighWatermarkMetadata(LogOffsetMetadata(newHighWatermark)) // 调用setter方法更新高水位值
    newHighWatermark // 返回新的高水位值
  }
   ...
   */
  def maybeIncrementHighWatermark(newHighWatermark: LogOffsetMetadata): Option[LogOffsetMetadata] = { // 使用场景：更新Leader副本的高水位值 Leader副本的高水位值更新是有条件：某些情况下不会去更新高水位值
    if (newHighWatermark.messageOffset > logEndOffset) // 确保新的高水位值不能超过LEO
    ...
    lock.synchronized {
      ...
      if (oldHighWatermark.messageOffset < newHighWatermark.messageOffset ||
        (oldHighWatermark.messageOffset == newHighWatermark.messageOffset && oldHighWatermark.onOlderSegment(newHighWatermark))) { // 1，新的高水位值要比老的高水位值大 维持单调增加的特性 2，新的高水位值在新日志段上也可以执行更新高水位的操作
   ...
   */
  private def fetchHighWatermarkMetadata: LogOffsetMetadata = { // 获取高水位值以及高水位值的其他元数据信息：日志段的起始位移和物理位置
    ...
    if (offsetMetadata.messageOffsetOnly) { // 没有获取到完整的高水位元数据
      lock.synchronized {
        val fullOffset = convertToOffsetMetadataOrThrow(highWatermark) // 通过读日志文件的方式把完整的高水位元数据信息读出来
        updateHighWatermarkMetadata(fullOffset) // 更新高水位对象
        fullOffset
      }
    } else { // 获取到的高水位元数据是完整的 直接返回
      offsetMetadata
    }
  }

  private def updateHighWatermarkMetadata(newHighWatermark: LogOffsetMetadata): Unit = { // setter设置高水位值
    if (newHighWatermark.messageOffset < 0) // 高水位值不能是负数
      throw new IllegalArgumentException("High watermark offset should be non-negative")

    lock synchronized { // jvm monitor锁
      highWatermarkMetadata = newHighWatermark // 赋值新的高水位值
      producerStateManager.onHighWatermarkUpdated(newHighWatermark.messageOffset) // 处理事务状态管理器的高水位值更新
      maybeIncrementFirstUnstableOffset() // First Unstable Offset事务机制的一部分
    }
  ...
  private def removeTempFilesAndCollectSwapFiles(): Set[File] = {
    ...
    def deleteIndicesIfExist(baseFile: File, suffix: String = ""): Unit = { // 删除日志文件对应的索引文件
    ...
    for (file <- dir.listFiles if file.isFile) { // 遍历分区日志路径下的所有文件
      if (!file.canRead) // 如果不可读直接抛异常
        throw new IOException(s"Could not read file $file")
      val filename = file.getName
      if (filename.endsWith(DeletedFileSuffix)) { // 如果文件以.deleted结尾说明是上次Failure遗留的文件 直接删除
        debug(s"Deleting stray temporary file ${file.getAbsolutePath}")
        Files.deleteIfExists(file.toPath)
      } else if (filename.endsWith(CleanedFileSuffix)) { // .cleaned结尾
        minCleanedFileOffset = Math.min(offsetFromFileName(filename), minCleanedFileOffset)
        cleanFiles += file
      } else if (filename.endsWith(SwapFileSuffix)) { // .swap结尾
        // we crashed in the middle of a swap operation, to recover:
        // if a log, delete the index files, complete the swap operation later
        // if an index just delete the index files, they will be rebuilt
        val baseFile = new File(CoreUtils.replaceSuffix(file.getPath, SwapFileSuffix, ""))
        info(s"Found file ${file.getAbsolutePath} from interrupted swap operation.")
        if (isIndexFile(baseFile)) { // 如果.swap原来是索引文件 删除原来的索引文件
          deleteIndicesIfExist(baseFile)
        } else if (isLogFile(baseFile)) { // 如果.swap原来是日志文件 删除掉原来的索引文件
          deleteIndicesIfExist(baseFile)
    ...
    val (invalidSwapFiles, validSwapFiles) = swapFiles.partition(file => offsetFromFile(file) >= minCleanedFileOffset) // 从待恢复swap集合中找出那些起始位移值大于minCleanedFileOffset值的文件 直接删除这些无效的.swap文件
   ...
   */
  private def loadSegmentFiles(): Unit = { // 清空已有日志段集合并重新加载日志段文件
    // load segments in ascending order because transactional data from one segment may depend on the
    // segments that come before it
    for (file <- dir.listFiles.sortBy(_.getName) if file.isFile) { // 按照日志段文件名中的位移值正序排列 然后遍历每个文件
      if (isIndexFile(file)) { // 如果是索引文件
        // if it is an index file, make sure it has a corresponding .log file
        val offset = offsetFromFile(file)
        val logFile = Log.logFile(dir, offset)
        if (!logFile.exists) { // 确保存在对应的日志文件 否则打个log 并删除该索引文件
        ...
        val segment = LogSegment.open(dir = dir,
          ...
          config,
          time = time,
          fileAlreadyExists = true) // 创建对应的LogSegment对象实例 加入segments中
   ...
   */
  private def completeSwapOperations(swapFiles: Set[File]): Unit = { // 处理有效.swap文件集合
    for (swapFile <- swapFiles) { // 遍历所有的有效.swap文件
      ...
      val swapSegment = LogSegment.open(swapFile.getParentFile,
        ...
        config,
        time = time,
        fileSuffix = SwapFileSuffix) // 创建对应的LogSegment实例
      info(s"Found log file ${swapFile.getPath} from interrupted swap operation, repairing.")
      recoverSegment(swapSegment) // 执行日志段恢复操作
      ...
      replaceSegments(Seq(swapSegment), oldSegments.toSeq, isRecoveredSwapFile = true) // 将生成的.swap文件加到日志中 删除掉swap之前的日志段
    }
  }
  ...
  private def loadSegments(): Long = {
    ...
    val swapFiles = removeTempFilesAndCollectSwapFiles() // 移除上次Failure遗留下来的各种临时文件: .cleaned、.swap、.deleted文件等
    ...
    retryOnOffsetOverflow {
      ...
      segments.clear()
      loadSegmentFiles() // 清空已有日志段集合并重新加载日志段文件
    }

    // Finally, complete any interrupted swap operations. To be crash-safe,
    // log files that are replaced by the swap segment should be renamed to .deleted
    // before the swap file is restored as the new segment file.
    completeSwapOperations(swapFiles) // 处理上面返回的有效.swap文件集合
  ...
  private def updateLogEndOffset(offset: Long): Unit = { // LEO对象被更新的时机：1，Log对象初始化时 2，写入新消息时 3，Log对象发生日志切分Log Roll 4，日志截断
  ...
  private def updateLogStartOffset(offset: Long): Unit = { // LogStartOffset的更新 更新时机：1，Log对象初始化时 2，日志截断 3，Follower副本同步时 4，删除日志段 5，删除消息时
    logStartOffset = offset

    if (highWatermark < offset) { // LogStartOffset <= highWatermark <= LEO
  ...
  private[log] def recoverLog(): Long = {
    ...
    if (!hadCleanShutdown) { // 如果不存在以.kafka_cleanshutdown结尾的文件(该后缀通常都不存在)
      // okay we need to actually recover this log
      val unflushed = logSegments(this.recoveryPoint, Long.MaxValue).iterator // 获取到上次恢复点以外的所有unflushed日志段对象
      var truncated = false

      while (unflushed.hasNext && !truncated) { // 遍历这些unflushed日志段对象
        ...
        if (truncatedBytes > 0) { // 如果无效的消息导致被截断的字节数不为0 直接删除剩余的日志段对象
    ...
    if (logSegments.nonEmpty) { // 这些都做完之后 如果日志段集合不为空
    ...
    if (logSegments.isEmpty) { // 上面执行完如果日志段集合为空了
      ...
      addSegment(LogSegment.open(dir = dir,
        ...
        fileAlreadyExists = false,
        initFileSize = this.initFileSize,
        preallocate = config.preallocate)) // 至少创建一个新的日志段 以logStartOffset为日志段的起始位移 并加入日志段集合中
    }

    recoveryPoint = activeSegment.readNextOffset // 更新上次恢复点属性并返回
    recoveryPoint
  }
  ...
  def appendAsLeader(records: MemoryRecords,
                     ...
                     interBrokerProtocolVersion: ApiVersion = ApiVersion.latestVersion): LogAppendInfo = { // 写Leader副本
   ...
   * @return Information about the appended messages including the first and last offset.
   */
  def appendAsFollower(records: MemoryRecords): LogAppendInfo = { // 写Follower副本
    append(records,
      origin = AppendOrigin.Replication,
  ...
  private def append(records: MemoryRecords,
    ...
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      val appendInfo = analyzeAndValidateRecords(records, origin, ignoreRecordSize) // 分析和验证待写入消息集合并返回校验结果

      // return if we have no valid messages or if this is a duplicate of the last appended entry
      if (appendInfo.shallowCount == 0) appendInfo // 如果不需要写入消息 就直接返回
      else {

        // trim any invalid bytes or partial messages before appending it to the on-disk log
        var validRecords = trimInvalidBytes(records, appendInfo) // 消息格式规整: 删除无效格式消息或无效字节
        ...
        lock synchronized {
          ...
          if (assignOffsets) { // 需要分配位移
            ...
            val offset = new LongRef(nextOffsetMetadata.messageOffset) // 使用当前LEO值作为待写入消息集合中第一条消息的位移值
            ...
            validRecords = validateAndOffsetAssignResult.validatedRecords // 更新校验结果对象类LogAppendInfo
            ...
            // format conversion)
            if (!ignoreRecordSize && validateAndOffsetAssignResult.messageSizeMaybeChanged) { // 验证消息 确保消息大小不超限
          ...
          // update the epoch cache with the epoch stamped onto the message by the leader // 更新Leader Epoch缓存
          validRecords.batches.forEach { batch =>
          ...
          if (validRecords.sizeInBytes > config.segmentSize) { // 确保消息大小不超限
            throw new RecordBatchTooLargeException(s"Message batch size is ${validRecords.sizeInBytes} bytes in append " +
              s"to partition $topicPartition, which exceeds the maximum configured segment size of ${config.segmentSize}.")
          }

          // maybe roll the log if this segment is full
          val segment = maybeRoll(validRecords.sizeInBytes, appendInfo) // 执行日志切分 当前日志段剩余容量可能无法容纳新消息集合 因此必须要创建一个新的日志段来保存待写入的所有消息
          ...
          val (updatedProducers, completedTxns, maybeDuplicate) = analyzeAndValidateProducerState(
            logOffsetMetadata, validRecords, origin) // 验证事务状态
          ...
          maybeDuplicate match {
            ...
            case None =>
              segment.append(largestOffset = appendInfo.lastOffset,
                ...
                records = validRecords) // 执行真正的消息写入操作 主要调用日志段对象的append()方法
              ...
              updateLogEndOffset(appendInfo.lastOffset + 1) // 成功写入日志段消息之后 更新LEO LEO永远指向下一条不存在的消息

              // update the producer state
              updatedProducers.values.foreach(producerAppendInfo => producerStateManager.update(producerAppendInfo)) // 更新事务状态
              ...
              if (unflushedMessages >= config.flushInterval) flush() // 是否需要手动落盘 一般情况下不需要设置Broker端的参数log.flush.interval.messages 落盘操作交由操作系统完成 但某些情况下可以设置该参数来确保高可靠性
          }
          appendInfo // 返回写入结果
        }
      }
  ...
  private def analyzeAndValidateRecords(records: MemoryRecords,
                                        origin: AppendOrigin,
                                        ignoreRecordSize: Boolean): LogAppendInfo = { // 对消息集合进行校验 并生成对应的LogAppendInfo对象
    ...
    records.batches.forEach { batch =>
      ...
      if (batch.magic >= RecordBatch.MAGIC_VALUE_V2 && origin == AppendOrigin.Client && batch.baseOffset != 0) // 消息格式Version 2的消息批次 起始位移值必须从0开始
      ...
      if (lastOffset >= batch.lastOffset) // 一旦出现当前lastOffset不小于下一个batch的lastOffset 说明上一个batch中有消息的位移值大于后面batch的消息 这样的话就违反了位移值的单调递增性
        monotonic = false

      // update the last offset seen
      lastOffset = batch.lastOffset // 使用当前batch最后一条消息的位移值去更新lastOffset

      // Check if the message sizes are valid.
      val batchSize = batch.sizeInBytes // 检验消息批次总字节数大小是否超限 即是否大于Broker端参数max.message.bytes值
      ...
      if (!batch.isValid) { // 执行消息批次检验 包括格式是否正确以及CRC校验
        brokerTopicStats.allTopicsStats.invalidMessageCrcRecordsPerSec.mark()
        throw new CorruptRecordException(s"Record is corrupt (stored crc = ${batch.checksum()}) in topic partition $topicPartition.")
      }

      if (batch.maxTimestamp > maxTimestamp) { // 更新maxTimestamp字段和offsetMaxTimestamp字段
        maxTimestamp = batch.maxTimestamp
        offsetOfMaxTimestamp = lastOffset
      }

      shallowMessageCount += 1 // 累加消息批次计数器以及有效字节数，更新shallowMessageCount字段
      validBytesCount += batchSize

      val messageCodec = CompressionCodec.getCompressionCodec(batch.compressionType.id) // 从消息皮从中获取压缩器类型
      if (messageCodec != NoCompressionCodec)
        sourceCodec = messageCodec
    }

    // Apply broker-side compression if any
    val targetCodec = BrokerCompressionCodec.getTargetCompressionCodec(config.compressionType, sourceCodec) // 获取Broker端设置的压缩器类型 即Broker端参数compression.type的值 该参数默认值是producer 表示sourceCodec用的什么压缩器 targetCodec就用什么
    LogAppendInfo(firstOffset, lastOffset, maxTimestamp, offsetOfMaxTimestamp, RecordBatch.NO_TIMESTAMP, logStartOffset,
      RecordConversionStats.EMPTY, sourceCodec, targetCodec, shallowMessageCount, validBytesCount, monotonic, lastOffsetOfFirstBatch) // 最后生成LogAppendInfo对象返回
  }
   ...
   *
   * @param startOffset The offset to begin reading at // 从Log对象的哪个位移值开始读消息
   * @param maxLength The maximum number of bytes to read // 最多能读取多少字节
   ...
   * @param minOneMessage If this is true, the first message will be returned even if it exceeds `maxLength` (if one exists) // 是否允许至少读一条消息 如果消息很大 超过了maxLength 正常情况下read方法永远也不会返回任何消息 但如果设置了该参数为true read方法就保证至少能够返回一条消息
  ...
  def read(startOffset: Long,
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      val endOffsetMetadata = nextOffsetMetadata // 读取消息时没有使用Monitor锁同步机制 因此这里用本地变量的方式把LEO对象保存起来 避免争用
      val endOffset = endOffsetMetadata.messageOffset
      var segmentEntry = segments.floorEntry(startOffset) // 找到startOffset值所在的日志段对象

      // return error on attempt to read beyond the log end offset or read below log start offset
      if (startOffset > endOffset || segmentEntry == null || startOffset < logStartOffset) // 满足以下3个条件之一就被视为消息越界 要读取的消息不在Log对象中 1，要读取的消息位移超过了LEO值 2，没找到对应的日志段对象 3，要读取的消息在Log Start Offset之下 同样是对象不可见的消息
        throw new OffsetOutOfRangeException(s"Received request for offset $startOffset for partition $topicPartition, " +
          s"but we only have log segments in the range $logStartOffset to $endOffset.")

      val maxOffsetMetadata = isolation match { // 查看以下读取隔离级别 普通消费者能够看到[LogStartOffset, 高水位值)之间的消息 事务型消费者能够看到[LogStartOffset, LogStableOffset]之间的消息
      ...
      if (startOffset == maxOffsetMetadata.messageOffset)
      ...
      else if (startOffset > maxOffsetMetadata.messageOffset) // 如果要读取的起始位置超过了能读取的最大位置 返回空的消息集合 因为没发读取任何消息
        ...
        while (!done) { // 开始遍历日志段对象 直到读出来东西或者读到日志段末尾
          ...
          fetchDataInfo = segment.read(startOffset, maxLength, maxPosition, minOneMessage) // 调用日志段对象的read方法执行真正的读取消息操作
          if (fetchDataInfo != null) {
        ...
        if (fetchDataInfo != null) fetchDataInfo
          ...
          FetchDataInfo(nextOffsetMetadata, MemoryRecords.EMPTY) // 已经读到日志末尾还是没有数据返回 只能返回空的消息集合
        }
      }
    }
  }
  ...
  private def deleteOldSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean,
                                reason: SegmentDeletionReason): Int = { // 3个留存策略本质都是调用这个方法
    lock synchronized {
      val deletable = deletableSegments(predicate) // 使用传入的函数计算哪些日志段对象能够被删除
      if (deletable.nonEmpty)
        deleteSegments(deletable, reason) // 删除日志段
      else
        0
    }
  }

  private def deleteSegments(deletable: Iterable[LogSegment], reason: SegmentDeletionReason): Int = { // 日志段删除
    maybeHandleIOException(s"Error while deleting segments for $topicPartition in dir ${dir.getParent}") {
      val numToDelete = deletable.size
      if (numToDelete > 0) {
        // we must always have at least one segment, so if we are going to delete all the segments, create a new one first
        if (segments.size == numToDelete) // 不允许删除所有日志段对象 如果一定要做 先创建一个新的再把前N个删掉
          roll()
        lock synchronized {
          ...
          maybeIncrementLogStartOffset(segments.firstEntry.getValue.baseOffset, SegmentDeletion) // 尝试更新日志的LogStartOffset值
        }
      }
      numToDelete
    }
   ...
   */
  private def deletableSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean): Iterable[LogSegment] = { // 找出能够被删除的日志段
    if (segments.isEmpty) { // 当前没有任何日志段对象
      Seq.empty
    } else {
      val deletable = ArrayBuffer.empty[LogSegment]
      var segmentEntry = segments.firstEntry
      while (segmentEntry != null) { // 从具有最小起始位移值的日志段对象开始遍历直到满足以下3个条件之一便停止 1，测定条件函数predicate=false 2，扫描到包含Log对象高水位值所在的日志段对象 3，最新的日志段对象不包含任何消息 遍历过程中同时不满足上面3个条件的便可以直接删除 最新日志段对象是segments中key值最大对应的日志段 也就是ActiveSegment 完全为空的ActiveSegment如果被允许删除后面还要重建它 所以不允许删除为空的ActiveSegment
        val segment = segmentEntry.getValue
        val nextSegmentEntry = segments.higherEntry(segmentEntry.getKey)
   ...
   */
  def deleteOldSegments(): Int = { // 删除操作 kafka留存策略：基于时间维度、基于空间维度、基于LogStartOffset维度 根据一定规则决定哪些日志段可以删除
    if (config.delete) {
      deleteRetentionMsBreachedSegments() + deleteRetentionSizeBreachedSegments() + deleteLogStartOffsetBreachedSegments() // 这3个方法对应着3个留存策略
    } else {
   ...
   */
  @threadsafe
  def addSegment(segment: LogSegment): LogSegment = this.segments.put(segment.baseOffset, segment) // 添加到segments这个Map中
 ...
 */
object Log { // 定义常量及辅助方法

  /** a log file */
  val LogFileSuffix = ".log" // 消息日志文件

  /** an index file */
  val IndexFileSuffix = ".index" // 位移索引文件

  /** a time index file */
  val TimeIndexFileSuffix = ".timeindex" // 时间戳索引文件

  val ProducerSnapshotFileSuffix = ".snapshot" // kafka为幂等型或事务型producer所做的快照文件

  /** an (aborted) txn index */
  val TxnIndexFileSuffix = ".txnindex" // 已终止事务索引文件

  /** a file that is scheduled to be deleted */
  val DeletedFileSuffix = ".deleted" // 删除日志段操作所创建的文件 删除日志段文件是异步操作 Broker端把日志段文件从.log后缀修改为.deleted后缀

  /** A temporary file that is being used for log cleaning */
  val CleanedFileSuffix = ".cleaned" // Compaction操作的产物

  /** A temporary file used when swapping files into the log */
  val SwapFileSuffix = ".swap" // Compaction操作的产物
  ...
  val DeleteDirSuffix = "-delete" // 应用于文件夹 删除一个主题的时候 主题的分区文件夹会被加上这个后缀

  /** a directory that is used for future partition */
  val FutureDirSuffix = "-future" // 用于变更主题分区文件夹地址
   ...
   * @return The filename
   */
  def filenamePrefixFromOffset(offset: Long): String = { // 通过给定的位移值计算出对应的日志段文件名 kafka日志文件固定是20位的长度 该方法是用前面补0的方式把给定位移值扩充成一个固定20位长度的字符串
...
object LogMetricNames { // Log对象监控指标
