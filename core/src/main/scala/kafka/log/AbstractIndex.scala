...
abstract class AbstractIndex(@volatile private var _file: File, val baseOffset: Long, val maxIndexSize: Int = -1,
                             val writable: Boolean) extends Closeable { // 定义了最顶层的抽象类 封装了所有索引类型的公共操作 定义了4个属性字段 该类是个抽象基类 它的所有子类自动继承了这4个字段 也就是 kafka所有类型的索引对象都定义了这些属性 file:索引文件，每个索引对象在磁盘都对应了一个索引文件，这个字段是var类型，说明它是可以被修改的，kafka自1.1.0版本之后允许迁移底层的日志路径，所以索引文件自然是可以更换的 baseOffset:起始位移值，索引对象对应日志段对象的起始位移值 maxIndexSize:索引文件最大字节数，它控制索引文件的最大长度，kafka源码传入该参数的值是Broker端参数化segment.index.bytes的值即10M，也就是说默认情况下kafka索引文件大小都是10M writeable:索引文件打开方式，true表示读写方式打开，false表示只读方式打开
 ...
 */
  ...
  @volatile
  protected var mmap: MappedByteBuffer = { // kafka索引底层的实现原理：内存映射文件，即java中的MappedByteBuffer 使用内存映射文件的主要又是在于它有很高的IO性能 特别是对于索引这样的小文件来说 由于文件内存被直接映射到一段虚拟内存上 访问内存映射文件的速度要快于普通的读写文件速度 另外很多操作系统中 这段映射的内存区域实际上就是内核的页缓存 这就意味着里面的数据不需要重复拷贝到用户态空间 避免了很多不必要的时间空间消耗
    val newlyCreated = file.createNewFile() // 创建索引文件
    val raf = if (writable) new RandomAccessFile(file, "rw") else new RandomAccessFile(file, "r") // 以writeable指定的方式(读写方式或者读方式)打开索引文件
    try {
      /* pre-allocate the file if necessary */
      if(newlyCreated) {
        if(maxIndexSize < entrySize) // 预设的索引文件不能太小 如果连一个索引都保存不了直接抛异常
          throw new IllegalArgumentException("Invalid max index size: " + maxIndexSize)
        raf.setLength(roundDownToExactMultiple(maxIndexSize, entrySize)) // 设置索引文件长度 roundDownToExactMultiple计算的是不超过maxIndexSize的最大整数倍entrySize 比如maxIndexSize=1234567 entrySize=8 那么调增后的文件长度为1234560
      }

      /* memory-map the file */
      _length = raf.length() // 更新索引长度字段_length
      val idx = { // 创建MappedByteBuffer对象
        if (writable)
      ...
      if(newlyCreated) // 如果是新创建的索引文件 将MappedByteBuffer对象的当前位置设置为0 如果索引文件已经存在 将MappedByteBuffer对象的当前位置设置成最后一个索引项所在的位置
        idx.position(0)
      else
        // if this is a pre-existing index, assume it is valid and set position to last entry
        idx.position(roundDownToExactMultiple(idx.limit(), entrySize))
      idx // 返回创建的MappedByteBuffer对象
    } finally {
   ...
   */
  protected def parseEntry(buffer: ByteBuffer, n: Int): IndexEntry // n表示要查找给定ByteBuffer中保存的第n个索引项(第n个槽)
  ...
  private def indexSlotRangeFor(idx: ByteBuffer, target: Long, searchEntity: IndexSearchType): (Int, Int) = {
    ...
    if(_entries == 0) // 如果索引为空 直接返回(-1,-1)对
      return (-1, -1)

    def binarySearch(begin: Int, end: Int) : (Int, Int) = { // 封装原版bs算法
    ...
    val firstHotEntry = Math.max(0, _entries - 1 - _warmEntries) // 确认热区首个索引项位于哪个槽 _warmEntries就是所谓的分割线 目前固定为8 如果是OffsetIndex _warmEntries=8192/8=1024 即第1024个槽 如果TimeIndex _warmEntries=8192/12=682 即第682个槽
    // check if the target offset is in the warm section of the index
    if(compareIndexEntry(parseEntry(idx, firstHotEntry), target, searchEntity) < 0) { // 判断target位移值在热区还是冷区
      return binarySearch(firstHotEntry, _entries - 1) // 如果在热区就搜索热区
    }

    // check if the target offset is smaller than the least offset
    if(compareIndexEntry(parseEntry(idx, 0), target, searchEntity) > 0) // 确保target位移值不能小于当前最小位移值
      return (-1, 0)

    binarySearch(0, firstHotEntry) // 如果在冷区就搜索冷区
  }
