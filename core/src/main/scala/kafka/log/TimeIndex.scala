...
class TimeIndex(_file: File, baseOffset: Long, maxIndexSize: Int = -1, writable: Boolean = true)
    extends AbstractIndex(_file, baseOffset, maxIndexSize, writable) { // 定义时间戳索引 保存 时间戳-相对位移值 时间戳使用一个长整型来保存 相对位移值使用Integer来保存 TimeIndex单个索引项需要占用12个字节 在保存同等数量索引项的基础上 TimeIndex会比OffsetIndex占用更多的磁盘空间
  import TimeIndex._
  ...
  def maybeAppend(timestamp: Long, offset: Long, skipFullCheck: Boolean = false): Unit = {
    inLock(lock) {
      if (!skipFullCheck)
        require(!isFull, "Attempt to append to a full time index (size = " + _entries + ").") // 如果索引文件已经写满 直接抛异常
