 ...
 */
@nonthreadsafe
class TransactionIndex(val startOffset: Long, @volatile private var _file: File) extends Logging { // 定义事务索引 为已终止事务Aborted Transaction保存重要的元数据信息 只有启用kafka事务后 这个索引才有可能出现
