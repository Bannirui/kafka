    ...
    try {
      ...
      if (canStartup) {
        ...
        socketServer = new SocketServer(config, metrics, time, credentialProvider,
          allowDisabledApis = config.metadataQuorumEnabled) // 创建SocketServer组件
        socketServer.startup(startProcessingRequests = false) // 启动SocketServer 但不启动Processor线程
        ...
        socketServer.startProcessingRequests(authorizerFutures) // 启动Data plane和Control plane的所有线程
