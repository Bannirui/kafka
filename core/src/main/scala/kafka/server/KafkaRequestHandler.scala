 ...
 */
class KafkaRequestHandler(id: Int, // IO线程序号
                          brokerId: Int, // 所在Broker序号 即broker.id
                          ...
                          time: Time) extends Runnable with Logging { // kafka IO线程 用于执行请求处理逻辑 请求处理线程类 每个请求处理线程实例 负责从SocketServer的RequestChannel的请求队列中获取请求对象 并进行处理
  this.logIdent = "[Kafka Request Handler " + id + " on Broker " + brokerId + "], "
  private val shutdownComplete = new CountDownLatch(1)
  @volatile private var stopped = false

  def run(): Unit = {
    while (!stopped) { // 只要线程尚未关闭就循环运行处理逻辑
      ...
      val req = requestChannel.receiveRequest(300) // 从请求队列中获取下一个待处理的请求
      val endTime = time.nanoseconds
      val idleTime = endTime - startSelectTime // 统计线程空闲时间
      aggregateIdleMeter.mark(idleTime / totalHandlerThreads.get) // 更新线程空闲百分比指标

      req match {
        case RequestChannel.ShutdownRequest => // 关闭线程请求
          debug(s"Kafka request handler $id on broker $brokerId received shut down command")
          shutdownComplete.countDown() // 关闭线程
          return

        case request: RequestChannel.Request => // 普通线程
          try {
            request.requestDequeueTimeNanos = endTime
            trace(s"Kafka request handler $id on broker $brokerId handling request $request")
            apis.handle(request) // 由于KafkaApis.handle方法执行相应处理逻辑
          } catch {
            case e: FatalExitError => // 出现严重错误 立即关闭线程
              shutdownComplete.countDown()
              Exit.exit(e.statusCode)
            case e: Throwable => error("Exception when handling request", e) // 如果是普通异常 记录日志就行
          } finally {
...
class KafkaRequestHandlerPool(val brokerId: Int, // 所属Broker的序号 即broker.id
                              val requestChannel: RequestChannel, // SocketServer组件下的RequestChannel对象
                              val apis: ApiRequestHandler, // KafkaApis类 实际请求处理逻辑类
                              time: Time,
                              numThreads: Int, // IO线程池初始大小
                              requestHandlerAvgIdleMetricName: String,
                              logAndThreadNamePrefix : String) extends Logging with KafkaMetricsGroup { // IO线程池 创建、维护和管理所有IO线程 请求处理线程池 负责创建、维护、管理和销毁下辖的请求处理线程

  private val threadPoolSize: AtomicInteger = new AtomicInteger(numThreads) // IO线程池大小
  /* a meter to track the average free capacity of the request handlers */
  private val aggregateIdleMeter = newMeter(requestHandlerAvgIdleMetricName, "percent", TimeUnit.NANOSECONDS)

  this.logIdent = "[" + logAndThreadNamePrefix + " Kafka Request Handler on Broker " + brokerId + "], "
  val runnables = new mutable.ArrayBuffer[KafkaRequestHandler](numThreads) // IO线程池
  ...
  def resizeThreadPool(newSize: Int): Unit = synchronized { // 把IO线程池的线程数重设为指定的数值
    val currentSize = threadPoolSize.get // 当前线程数量
  ...
  def shutdown(): Unit = synchronized {
    ...
    for (handler <- runnables)
      handler.awaitShutdown() // 调用awaitShutdown方法等待关闭完成 run方法一旦调用countDown方法 这里将接触等待状态
    info("shut down completely")
  }
}

class BrokerTopicMetrics(name: Option[String]) extends KafkaMetricsGroup { // 定义Broker端主题相关监控指标管理类
...
object BrokerTopicStats { // 封装如MessageInPerSec等主题相关监控指标
...
class BrokerTopicStats extends Logging { // 定义Broker端主题相关监控指标的管理操作
