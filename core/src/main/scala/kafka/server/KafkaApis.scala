...
class KafkaApis(val requestChannel: RequestChannel, // 请求通道
                ...
                val zkClient: KafkaZkClient, // zk客户端程序 kafka依赖于该类实现与zk的交互
  ...
  override def handle(request: RequestChannel.Request): Unit = {
    try {
      ...
      request.header.apiKey match { // 根据请求头部信息中的apiKey字段判断属于哪类请求 然后调用相应的handle方法
      ...
      case e: FatalExitError => throw e // 如果是严重错误 就抛出异常
      case e: Throwable => handleError(request, e) // 如果是普通异常的话 就记录错误日志
    } finally {
      ...
      if (request.apiLocalCompleteTimeNanos < 0) // 记录以下请求本地完成时间 即Broker处理完该请求的时间
  ...
  private[server] def authorize(requestContext: RequestContext,
                                ...
                                refCount: Int = 1): Boolean = { // 授权检验
    authorizer.forall { authZ =>
      val resource = new ResourcePattern(resourceType, resourceName, PatternType.LITERAL) // 获取待鉴权的资源类型 常见的资源类型如TOPIC、GROUP、CLUSTER
      val actions = Collections.singletonList(new Action(operation, resource, refCount, logIfAllowed, logIfDenied))
      authZ.authorize(requestContext, actions).get(0) == AuthorizationResult.ALLOWED // 返回鉴权结果 是ALLOWED还是DENIED
    }
  }
