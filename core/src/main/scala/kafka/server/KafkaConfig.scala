...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean, dynamicConfigOverride: Option[DynamicBrokerConfig])
  ...
  private def getControlPlaneListenerNameAndSecurityProtocol: Option[(ListenerName, SecurityProtocol)] = {
    Option(getString(KafkaConfig.ControlPlaneListenerNameProp)) match { // 查看Broker端参数control.plane.listener.name值 即是否启用了control plane监听器
      case Some(name) => // 如果启用了
        val listenerName = ListenerName.normalised(name)
        val securityProtocol = listenerSecurityProtocolMap.getOrElse(listenerName,
          throw new ConfigException(s"Listener with ${listenerName.value} defined in " +
            s"${KafkaConfig.ControlPlaneListenerNameProp} not found in ${KafkaConfig.ListenerSecurityProtocolMapProp}.")) // 必须同时设置Broker端参数listener.security.protocol.map 并从该参数值中提取出该监听器对应的安全认证协议
        Some(listenerName, securityProtocol) // 返回<监听器名称, 安全认证协议>对

      case None => None // 如果没有设置该参数值 直接返回None 说明没有启用control plane监听器
   }
  }
