...
class ControllerContext {
  ...
  var offlinePartitionCount = 0 // 离线分区计数器 该字段统计集群中所有离线或处于不可用状态的主题分区数量 所谓的不可用状态：Leader=-1
  var preferredReplicaImbalanceCount = 0
  val shuttingDownBrokerIds = mutable.Set.empty[Int] // 关闭中Broker的Id列表 当Controller在管理集群Broker的时候 它需要依靠这个字段来甄别Broker当前是否已经关闭 因为处于关闭状态的Broker不适合执行某些操作：比如分区重分配、主题删除
  private val liveBrokers = mutable.Set.empty[Broker] // 当前运行中Broker对象列表 每个Broker对象就是一个<id, EndPoint, 机架信息>的三元组
  private val liveBrokerEpochs = mutable.Map.empty[Int, Long] // 运行中Broker Epoch列表 kafka使用Epoch数据防止Zombie Broker 即一个非常老的Broker被选举成为Controller
  var epoch: Int = KafkaController.InitialControllerEpoch // Controller当前的Epoch值
  var epochZkVersion: Int = KafkaController.InitialControllerEpochZkVersion // Controller对应zk节点的Epoch值

  val allTopics = mutable.Set.empty[String] // 集群主题列表 每当有主题增减 Controller就会更新这个字段
  ...
  def updateBrokerMetadata(oldMetadata: Broker, newMetadata: Broker): Unit = { // 每当新增或移除已有Broker时 zk就会更新其保存的Broker数据 从而引发Controller修改元数据 也就是会调用这个方法来增减Broker列表中的对象
  ...
  private def updatePartitionStateMetrics(partition: TopicPartition,
                                          ...
                                          targetState: PartitionState): Unit = { // 更新offlinePartitionCount元数据
    if (!isTopicDeletionInProgress(partition.topic)) { // 如果该主题当前并未处于删除中状态
      if (currentState != OfflinePartition && targetState == OfflinePartition) { // 如果当前状态不是OfflinePartition 即离线状态 并且目标状态是离线状态 这个if语句判断是否要将该主题分区状态转换到离线状态
        offlinePartitionCount = offlinePartitionCount + 1
      } else if (currentState == OfflinePartition && targetState != OfflinePartition) { // 如果当前状态已经是离线状态 但targetState不是 这个分支判断是否要将该主题分区状态转换到非离线状态
        offlinePartitionCount = offlinePartitionCount - 1
      }
    }
  }
