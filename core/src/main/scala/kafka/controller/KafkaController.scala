...
sealed trait ElectionTrigger // 选举触发器 这个选举不是指Controller选举 而是指主题分区副本的选取 即为哪些分区选择Leader副本
...
object KafkaController extends Logging { // 定义了常量和回调函数类型
...
class KafkaController(val config: KafkaConfig, // kafka配置信息 通过它可以拿到Broker端的所有参数的值
                      zkClient: KafkaZkClient, // zk客户端 Controller与zk的所有交互都通过该属性完成
                      time: Time, // 提供时间服务的工具类
                      metrics: Metrics,
                      initialBrokerInfo: BrokerInfo, // Broker节点信息 包括主机名、端口号、所用监听器
                      initialBrokerEpoch: Long, // Broker Epoch值 用于隔离老Controller发送的请求
                      tokenManager: DelegationTokenManager, // 实现Delegation token管理的工具类 Delegation token是一种轻量级的认证管理工具
  ...
  val controllerContext = new ControllerContext // 集群元数据类 保存集群所有的元数据
  var controllerChannelManager = new ControllerChannelManager(controllerContext, config, time, metrics,
    stateChangeLogger, threadNamePrefix) // Controller端通道管理器类 负责Controller向Broker发送请求

  // have a separate scheduler for the controller to be able to start and stop independently of the kafka server
  // visible for testing
  private[controller] val kafkaScheduler = new KafkaScheduler(1) // 线程调度器 当前唯一负责定期执行Leader重选举

  // visible for testing
  private[controller] val eventManager = new ControllerEventManager(config.brokerId, this, time,
    controllerContext.stats.rateAndTimeMetrics) // Controller事件管理器 负责管理事件处理线程
  ...
  val replicaStateMachine: ReplicaStateMachine = new ZkReplicaStateMachine(config, stateChangeLogger, controllerContext, zkClient,
    new ControllerBrokerRequestBatch(config, controllerChannelManager, eventManager, controllerContext, stateChangeLogger)) // 副本状态机 负责副本状态转换
  val partitionStateMachine: PartitionStateMachine = new ZkPartitionStateMachine(config, stateChangeLogger, controllerContext, zkClient,
    new ControllerBrokerRequestBatch(config, controllerChannelManager, eventManager, controllerContext, stateChangeLogger)) // 分区状态机 负责分区状态转换
  val topicDeletionManager = new TopicDeletionManager(config, controllerContext, replicaStateMachine,
    partitionStateMachine, new ControllerDeletionClient(this, zkClient)) // 主题删除管理器 负责删除主题及日志

  private val controllerChangeHandler = new ControllerChangeHandler(eventManager) // Controller节点zk监听器 监听/controller节点变更 包括节点创建、删除、数据变更
  private val brokerChangeHandler = new BrokerChangeHandler(eventManager) // Broker数量zk监听器
  private val brokerModificationsHandlers: mutable.Map[Int, BrokerModificationsHandler] = mutable.Map.empty // Broker信息变更zk监听器集合 监听Broker数据变更 比如Broker的配置信息发生变化
  private val topicChangeHandler = new TopicChangeHandler(eventManager) // 主题数量zk监听器
  private val topicDeletionHandler = new TopicDeletionHandler(eventManager) // 主题删除zk监听器 监听主题删除节点/admin/delete_topics的子节点数量变更
  private val partitionModificationsHandlers: mutable.Map[String, PartitionModificationsHandler] = mutable.Map.empty // 主题分区变更zk监听器 监控主题分区数据变更的监听器 比如：新增加了副本、分区更换了Leader副本
  private val partitionReassignmentHandler = new PartitionReassignmentHandler(eventManager) // 主题分区重分配zk监听器 监听分区副本重分配任务 一旦发现新提交的任务就为目标分区执行副本重分配
  private val preferredReplicaElectionHandler = new PreferredReplicaElectionHandler(eventManager) // Preferred Leader选举zk监听器 监听Preferred Leader选举任务 一旦发现新提交的任务 就为目标主题执行Preferred Leader选举
  private val isrChangeNotificationHandler = new IsrChangeNotificationHandler(eventManager) // ISR副本集合变更zk监听器 监听ISR副本集合变更 一旦被触发 就需要获取ISR发生变更的分区列表 然后更新Controller端对应的Leader和ISR缓存元数据
  private val logDirEventNotificationHandler = new LogDirEventNotificationHandler(eventManager) // 日志路径变更zk监听器 监听日志路径变更 一旦被触发 需要获取受影响的Broker列表 然后处理这些Broker上失效的日志路径

  @volatile private var activeControllerId = -1 // 当前Controller所在的Broker Id
  @volatile private var offlinePartitionCount = 0 // 离线分区总数
  ...
  @volatile private var topicsToDeleteCount = 0 // 待删除主题数
  @volatile private var replicasToDeleteCount = 0 // 待删除副本数
  @volatile private var ineligibleTopicsToDeleteCount = 0 // 暂时无法删除的主题数
  @volatile private var ineligibleReplicasToDeleteCount = 0 // 暂时无法删除的副本数
  ...
  def startup() = {
    ...
    }) // 注册zk状态变更监听器 用于监听zk会话过期
    eventManager.put(Startup) // 写入Startup事件到事件队列
    eventManager.start() // 启动ControllerEventThread线程 开始处理事件队列中的ControllerEvent
  }
   ...
   */
  private def onControllerResignation(): Unit = { // Controller卸任
    debug("Resigning")
    // de-register listeners // 以下5个方法为取消zk监听器的注册
    ...
    kafkaScheduler.shutdown() // 关闭kafka线程调度器 其实就是取消定期的Leader重选举
    offlinePartitionCount = 0 // 以下将统计字段全部清0
    ...
    if (tokenCleanScheduler.isStarted)
      tokenCleanScheduler.shutdown() // 关闭Token过期检查调度器

    // de-register partition ISR listener for on-going partition reassignment task
    unregisterPartitionReassignmentIsrChangeHandlers() // 取消分区重分配监听器的注册
    // shutdown partition state machine
    partitionStateMachine.shutdown() // 关闭分区状态机
    zkClient.unregisterZNodeChildChangeHandler(topicChangeHandler.path) // 取消主题变更监听器的注册
    unregisterPartitionModificationsHandlers(partitionModificationsHandlers.keys.toSeq) // 取消分区变更监听器的注册
    zkClient.unregisterZNodeChildChangeHandler(topicDeletionHandler.path) // 取消主题删除监听器的注册
    // shutdown replica state machine
    replicaStateMachine.shutdown() // 关闭副本状态机
    zkClient.unregisterZNodeChildChangeHandler(brokerChangeHandler.path) // 取消Broker变更监听器注册

    controllerChannelManager.shutdown() // 关闭Controller通道管理器
    controllerContext.resetContext() // 清空集群元数据
   ...
   */
  private def onBrokerStartup(newBrokers: Seq[Int]): Unit = { // 处理Broker启动的方法 Controller端应对集群新增Broker启动的方法
    info(s"New broker startup callback for ${newBrokers.mkString(",")}")
    newBrokers.foreach(controllerContext.replicasOnOfflineDirs.remove) // 移除元数据中新增Broker对应的副本集合
    ...
    sendUpdateMetadataRequest(existingBrokers.toSeq, Set.empty) // 给集群现有的Broker发送元数据更新请求 令它们同步集群当前的所有分区数据
    ...
    replicaStateMachine.handleStateChanges(allReplicasOnNewBrokers.toSeq, OnlineReplica) // 将新增Broker上的所有副本设置为Online状态 即可用状态
    ...
    if (replicasForTopicsToBeDeleted.nonEmpty) { // 重启之前暂停的主题删除操作
    ...
    registerBrokerModificationsHandler(newBrokers) // 为新增Broker注册BrokerModificationHandler监听器
  }
  ...
  private def onBrokerFailure(deadBrokers: Seq[Int]): Unit = {
    ...
    deadBrokers.foreach(controllerContext.replicasOnOfflineDirs.remove) // deadBrokers：给定的一组已终止运行的Broker id列表 更新Controller元数据信息 将给定Broker从元数据的replicasOnOfflineDirs中移除
    val deadBrokersThatWereShuttingDown =
      deadBrokers.filter(id => controllerContext.shuttingDownBrokerIds.remove(id)) // 找出这些Broker上的所有副本对象 将待移除Broker从元数据对象中处于已关闭状态的Broker列表中去除
    if (deadBrokersThatWereShuttingDown.nonEmpty)
      info(s"Removed ${deadBrokersThatWereShuttingDown.mkString(",")} from list of shutting down brokers.")
    val allReplicasOnDeadBrokers = controllerContext.replicasOnBrokers(deadBrokers.toSet) // 执行副本清扫工作
    onReplicasBecomeOffline(allReplicasOnDeadBrokers)

    unregisterBrokerModificationsHandler(deadBrokers) // 取消这些Broker上注册的zk监听器
  }

  private def onBrokerUpdate(updatedBrokerId: Int): Unit = {
    info(s"Broker info update callback for $updatedBrokerId")
    sendUpdateMetadataRequest(controllerContext.liveOrShuttingDownBrokerIds.toSeq, Set.empty) // 给集群所有Broker发送UpdateMetadataRequest 让它们去更新元数据
  }
  ...
  private def processStartup(): Unit = {
    zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler) // 注册ControllerChangeHandler zk监听器
    elect() // 执行Controller选举
  }
  ...
  private def maybeResign(): Unit = {
    val wasActiveBeforeChange = isActive // 非常关键的一步 判断是否需要执行卸任逻辑的重要依据 判断该Broker之前是否是Controller
    zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler) // 注册ControllerChangeHandler监听器
    activeControllerId = zkClient.getControllerId.getOrElse(-1) // 获取当前集群Controller所在的Broker Id 如果没有Controller就返回-1
    if (wasActiveBeforeChange && !isActive) { // 如果该Broker之前是Controller但现在不是了
      onControllerResignation() // 执行卸任逻辑
    }
  }

  private def elect(): Unit = { // Controller选举
    activeControllerId = zkClient.getControllerId.getOrElse(-1) // 获取当前Controller所在的Broker Id 如果Controller不存在就返回-1
    /*
     * We can get here during the initial startup and the handleDeleted ZK callback. Because of the potential race condition,
     * it's possible that the controller has already been elected when we get here. This check will prevent the following
     * createEphemeralPath method from getting into an infinite loop if this broker is already the controller.
     */
    if (activeControllerId != -1) { // 如果当前Controller已经被选出来了 直接返回
      debug(s"Broker $activeControllerId has been elected as the controller, so stopping the election process.")
      return
    }

    try {
      val (epoch, epochZkVersion) = zkClient.registerControllerAndIncrementControllerEpoch(config.brokerId) // 注册Controller相关信息 主要创建/Controller节点
      ...
      onControllerFailover() // 执行当选Controller的后续逻辑
    } catch {
      case e: ControllerMovedException =>
        maybeResign()
  ...
  private def processBrokerChange(): Unit = {
    if (!isActive) return // 如果Broker不是Controller 自然无权处理 直接返回
    val curBrokerAndEpochs = zkClient.getAllBrokerAndEpochsInCluster // 从zk中获取集群Broker列表
    val curBrokerIdAndEpochs = curBrokerAndEpochs map { case (broker, epoch) => (broker.id, epoch) }
    val curBrokerIds = curBrokerIdAndEpochs.keySet
    val liveOrShuttingDownBrokerIds = controllerContext.liveOrShuttingDownBrokerIds // 获取Controller当前保存的Broker列表
    val newBrokerIds = curBrokerIds.diff(liveOrShuttingDownBrokerIds) // 比较两个列表 获取新增Broker列表、待移除Broker列表、已重启Broker列表和当前运行中的Broker列表
    ...
    newBrokerAndEpochs.keySet.foreach(controllerChannelManager.addBroker) // 为每个新增Broker创建与之连接的通道管理器和底层的请求发送线程RequestSendThread
    bouncedBrokerIds.foreach(controllerChannelManager.removeBroker) // 为每个已重启的Broker移除他们现有的配套资源(通道管理器、RequestSendThread等) 并重新添加它们
    bouncedBrokerAndEpochs.keySet.foreach(controllerChannelManager.addBroker)
    deadBrokerIds.foreach(controllerChannelManager.removeBroker) // 为每个待移除Broker移除对应的配套资源

    if (newBrokerIds.nonEmpty) { // 为新增Broker执行更新Controller元数据和Broker启动逻辑
    ...
    if (bouncedBrokerIds.nonEmpty) { // 为已重启Broker执行添加逻辑 包含更新ControllerContext、执行Broker重启动逻辑
    ...
    if (deadBrokerIds.nonEmpty) { // 为待移除Broker执行移除ControllerContext和Broker终止逻辑
  ...
  private def processBrokerModification(brokerId: Int): Unit = { // Controller端监听Broker端信息的变更
    if (!isActive) return
    val newMetadataOpt = zkClient.getBroker(brokerId) // 获取目标Broker的详细数据 包括每套监听器配置的主机名、端口号以及所使用的安全协议等
    val oldMetadataOpt = controllerContext.liveOrShuttingDownBroker(brokerId) // 从元数据缓存中获取目标Broker的详细数据
    if (newMetadataOpt.nonEmpty && oldMetadataOpt.nonEmpty) {
      val oldMetadata = oldMetadataOpt.get
      val newMetadata = newMetadataOpt.get
      if (newMetadata.endPoints != oldMetadata.endPoints || !oldMetadata.features.equals(newMetadata.features)) { // 如果两者不想等 说明Broker数据发生了变更 那么 更新元数据缓存 以及执行onBrokerUpdate方法处理Broker更新的逻辑
  ...
  private def processTopicChange(): Unit = { // 主题创建或更新
    if (!isActive) return // 如果Controller已经关闭 直接返回
    val topics = zkClient.getAllTopicsInCluster(true) // 从zk中回去当前所有的主题列表
    val newTopics = topics -- controllerContext.allTopics // 找出当前元数据中不存在 zk中存在的主题 视为新增主题
    val deletedTopics = controllerContext.allTopics.diff(topics) // 找出当前元数据中存在 zk中不存在的主题 视为已删除主题
    controllerContext.setAllTopics(topics) // 更新Controller元数据

    registerPartitionModificationsHandlers(newTopics.toSeq) // 为新增主题和已删除主题执行后续处理操作 为新增主题注册分区变更监听器 分区变更监听器是监听主题分区变更的
    val addedPartitionReplicaAssignment = zkClient.getReplicaAssignmentAndTopicIdForTopics(newTopics) // 从zk中获取新增主题的副本分配情况
    deletedTopics.foreach(controllerContext.removeTopic) // 清除元数据中属于已删除主题的缓存项
    ...
    if (addedPartitionReplicaAssignment.nonEmpty) { // 调整新增主题所有分区以及所属所有副本的运行状态为上线状态
  ...
  private def processTopicDeletion(): Unit = {
    if (!isActive) return
    var topicsToBeDeleted = zkClient.getTopicDeletions.toSet // 从zk中获取待删除主题列表
    debug(s"Delete topics listener fired for topics ${topicsToBeDeleted.mkString(",")} to be deleted")
    val nonExistentTopics = topicsToBeDeleted -- controllerContext.allTopics // 找出不存在的主题列表
    if (nonExistentTopics.nonEmpty) {
      warn(s"Ignoring request to delete non-existing topics ${nonExistentTopics.mkString(",")}")
      zkClient.deleteTopicDeletions(nonExistentTopics.toSeq, controllerContext.epochZkVersion)
    }
    topicsToBeDeleted --= nonExistentTopics
    if (config.deleteTopicEnable) { // 如果delete.topic.enable参数设置为true
      if (topicsToBeDeleted.nonEmpty) {
        ...
        topicDeletionManager.enqueueTopicsForDeletion(topicsToBeDeleted) // 将待删除主题插入到删除等待集合交由TopicDeletionManager处理
      }
    } else { // 不允许删除主题
      // If delete topic is disabled remove entries under zookeeper path : /admin/delete_topics
      info(s"Removing $topicsToBeDeleted since delete topic is disabled")
      zkClient.deleteTopicDeletions(topicsToBeDeleted.toSeq, controllerContext.epochZkVersion) // 清除zk下/admin/delete_topics下的子节点
    }
  }
...
class TopicChangeHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler {
  override val path: String = TopicsZNode.path // zk节点：/brokers/topics

  override def handleChildChange(): Unit = eventManager.put(TopicChange) // 向事件队列写入TopicChange事件
}
...
class TopicDeletionHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler { // 删除主题 zk监听器
  override val path: String = DeleteTopicsZNode.path // zk节点：/admin/delete_topics

  override def handleChildChange(): Unit = eventManager.put(TopicDeletion) // 向事件队列写入TopicDelete事件
}
...
class ControllerChangeHandler(eventManager: ControllerEventManager) extends ZNodeChangeHandler {
  override val path: String = ControllerZNode.path // zk中Controller的节点路径 即/Controller

  override def handleCreation(): Unit = eventManager.put(ControllerChange) // 监听/Controller节点创建事件
  override def handleDeletion(): Unit = eventManager.put(Reelect) // 监听/Controller节点被删除事件
  override def handleDataChange(): Unit = eventManager.put(ControllerChange) // 监听/Controller节点数据变更事件
}
...
private[controller] class ControllerStats extends KafkaMetricsGroup { // Controller的一些统计信息
  val uncleanLeaderElectionRate = newMeter("UncleanLeaderElectionsPerSec", "elections", TimeUnit.SECONDS) // 统计每秒发生的Unclean Leader选举次数 通常情况下执行Unclean Leader选举可能会造成数据丢失 一般不建议开启它 一旦开启就需要时刻关注这个监控指标的值 确保Unclean Leader选举的速率维持在一个很低的水平 否则会出现很多数据丢失的情况

  val rateAndTimeMetrics: Map[ControllerState, KafkaTimer] = ControllerState.values.flatMap { state =>
    state.rateAndTimeMetricName.map { metricName =>
      state -> new KafkaTimer(newTimer(metricName, TimeUnit.MILLISECONDS, TimeUnit.SECONDS))
    }
  }.toMap // Controller事件通用的统计速率指标的方法 统计所有Controller状态的速率和事件信息 单位是毫秒 当前 Controller定义了很多事件：TopicDeletion是执行主题删除的Controller事件、ControllerChange是执行Controller重选举的事件

}

sealed trait ControllerEvent { // Controller事件 也就是事件队列中被处理的对象 每个ControllerEvent都定义了一个状态 Controller在处理具体的事件时候 会对状态进行相应的变更 这个状态是由ControllerState这个抽象类定义的
