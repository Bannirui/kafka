...
class ControllerChannelManager(controllerContext: ControllerContext,
  ...
  private def addNewBroker(broker: Broker): Unit = {
    val messageQueue = new LinkedBlockingQueue[QueueItem] // 为该Broker构造请求阻塞队列
    debug(s"Controller ${config.brokerId} trying to connect to broker ${broker.id}")
    val controllerToBrokerListenerName = config.controlPlaneListenerName.getOrElse(config.interBrokerListenerName)
    val controllerToBrokerSecurityProtocol = config.controlPlaneSecurityProtocol.getOrElse(config.interBrokerSecurityProtocol)
    val brokerNode = broker.node(controllerToBrokerListenerName) // 获取待连接Broker的节点对象信息
    ...
    val (networkClient, reconfigurableChannelBuilder) = {
      ...
      val networkClient = new NetworkClient(
        ...
        new ApiVersions,
        logContext
      ) // 创建NetworkClient实例 NetworkClient是kafka Clients工程封装的顶层网络客户端API 提供了丰富的方法实现网络层IO数据传输
      (networkClient, reconfigurableChannelBuilder)
    }
    val threadName = threadNamePrefix match {
      case None => s"Controller-${config.brokerId}-to-broker-${broker.id}-send-thread"
      case Some(name) => s"$name:Controller-${config.brokerId}-to-broker-${broker.id}-send-thread"
    } // 为这个RequestSentThread线程设置线程名称
    ...
    val requestThread = new RequestSendThread(config.brokerId, controllerContext, messageQueue, networkClient,
      brokerNode, config, time, requestRateAndQueueTimeMetrics, stateChangeLogger, threadName) // 创建RequestSendThread实例
    requestThread.setDaemon(false)

    val queueSizeGauge = newGauge(QueueSizeMetricName, () => messageQueue.size, brokerMetricTags(broker.id))

    brokerStateInfo.put(broker.id, ControllerBrokerStateInfo(networkClient, brokerNode, messageQueue,
      requestThread, queueSizeGauge, requestRateAndQueueTimeMetrics, reconfigurableChannelBuilder)) // 创建该Broker专属的ControllerBrokerStateInfo实例 并将其加入到brokerStateInfo统一管理
  }
...
class RequestSendThread(val controllerId: Int, // Controller所在的Broker的Id
  ...
  override def doWork(): Unit = { // 线程执行逻辑的方法
    ...
    try {
      ...
      while (isRunning && !isSendSuccessful) {
        ...
        try {
          if (!brokerReady()) { // 如果没有创建与目标Broker的TCP连接 或连接暂时不可用
            ...
            clientResponse = NetworkClientUtils.sendAndReceive(networkClient, clientRequest, time) // 发送请求 等待接收Response
      ...
      if (clientResponse != null) { // 如果接收到了Response
        ...
        if (api != ApiKeys.LEADER_AND_ISR && api != ApiKeys.STOP_REPLICA && api != ApiKeys.UPDATE_METADATA) // 此Response的请求类型必须是LeaderAndIsrRequest、StopReplicaRequest或UpdateMetadataRequest中的一种
