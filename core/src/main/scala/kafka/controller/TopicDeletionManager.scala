...
trait DeletionClient { // 负责实现删除主题以及后续的动作 比如更新元数据等
...
class ControllerDeletionClient(controller: KafkaController, zkClient: KafkaZkClient) extends DeletionClient { // 实现上面的DeletionClient接口
  override def deleteTopic(topic: String, epochZkVersion: Int): Unit = { // 删除给定主题 用于删除主题在zk上的所有痕迹
    zkClient.deleteTopicZNode(topic, epochZkVersion) // 删除/brokers/topics/<topic>节点
    zkClient.deleteTopicConfigs(Seq(topic), epochZkVersion) // 删除/config/topics/<topic>节点
    zkClient.deleteTopicDeletions(Seq(topic), epochZkVersion) // 删除/admin/delete_topics/<topic>节点
  }

  override def deleteTopicDeletions(topics: Seq[String], epochZkVersion: Int): Unit = { // 删除/admin/delete_topics下的给定topic子节点
    zkClient.deleteTopicDeletions(topics, epochZkVersion)
  }

  override def mutePartitionModifications(topic: String): Unit = { // 取消/brokers/topics/<topic>节点数据变更的监听
    controller.unregisterPartitionModificationsHandlers(Seq(topic))
  }

  override def sendMetadataUpdate(partitions: Set[TopicPartition]): Unit = { // 向集群Broker发送指定分区的元数据更新请求
    controller.sendUpdateMetadataRequest(controller.controllerContext.liveOrShuttingDownBrokerIds.toSeq, partitions) // 给集群所有Broker发送UpdateMetadataRequest 通知它们给定partitions的状态变换
  }
}
 ...
 * @param controller
 */
class TopicDeletionManager(config: KafkaConfig, // 配置类 保存Broker端参数
                           ...
                           client: DeletionClient) extends Logging { // 负责对指定主题执行删除操作 清除待删除主题在集群上的各类痕迹 主题删除管理器 定义了若干个方法维护主题删除前后集群状态的正确性 比如：什么时候才能删除主题、什么时候主题不能被删除、主题闪促会过程中要规避哪些操作
  ...
  private def completeDeleteTopic(topic: String): Unit = {
    ...
    client.mutePartitionModifications(topic) // 注销分区变更监听器 防止删除过程中因分区数据变更导致监听器被触发 引起状态不一致
    val replicasForDeletedTopic = controllerContext.replicasInState(topic, ReplicaDeletionSuccessful) // 获取该主题下处于ReplicaDeletionSuccessful状态的所有副本对象 即所有已经被成功删除的副本对象
    // controller will remove this replica from the state machine as well as its partition assignment cache
    replicaStateMachine.handleStateChanges(replicasForDeletedTopic.toSeq, NonExistentReplica) // 利用副本状态机将这些副本对象转换成NonExistentReplica状态 等同于在状态机中删除这些副本
    client.deleteTopic(topic, controllerContext.epochZkVersion) // 移除zk上关于该主题的一切痕迹
    controllerContext.removeTopic(topic) // 移除元数据缓存中关于该主题的一切痕迹
  }
  ...
  private def onTopicDeletion(topics: Set[String]): Unit = {
    ...
    if (unseenTopicsForDeletion.nonEmpty) {
      val unseenPartitionsForDeletion = unseenTopicsForDeletion.flatMap(controllerContext.partitionsForTopic) // 获取这些主题的所有分区对象
      partitionStateMachine.handleStateChanges(unseenPartitionsForDeletion.toSeq, OfflinePartition) // 将这些分区的状态依次调整为OfflinePartition和NonExistentPartition 等同于将这些分区从分区状态机中删除
      ...
      controllerContext.beginTopicDeletion(unseenTopicsForDeletion) // 把这些主题加到 已开启删除操作 的主题列表中
    }
    ...
    onPartitionDeletion(topics) // 分区删除操作执行底层的物理磁盘文件删除动作
  }
  ...
  private def resumeDeletions(): Unit = { // 重启主题删除操作过程
    val topicsQueuedForDeletion = Set.empty[String] ++ controllerContext.topicsToBeDeleted // 从元数据缓存中获取要删除的主题列表
    val topicsEligibleForRetry = mutable.Set.empty[String] // 待重试主题列表
    val topicsEligibleForDeletion = mutable.Set.empty[String] // 待删除主题列表

    if (topicsQueuedForDeletion.nonEmpty)
      info(s"Handling deletion for topics ${topicsQueuedForDeletion.mkString(",")}")

    topicsQueuedForDeletion.foreach { topic => // 遍历待删除主题
      ...
      if (controllerContext.areAllReplicasInState(topic, ReplicaDeletionSuccessful)) { // 如果该主题所有副本已经是ReplicaDeletionSuccessful状态 即该主题已经被删除
        ...
        completeDeleteTopic(topic) // 完成后续操作
        info(s"Deletion of topic $topic successfully completed")
      } else if (!controllerContext.isAnyReplicaInState(topic, ReplicaDeletionStarted)) { // 如果主题删除尚未开始并且主题当前无法执行删除
        // if you come here, then no replica is in TopicDeletionStarted and all replicas are not in
        // TopicDeletionSuccessful. That means, that either given topic haven't initiated deletion
        // or there is at least one failed replica (which means topic deletion should be retried).
        if (controllerContext.isAnyReplicaInState(topic, ReplicaDeletionIneligible)) {
          topicsEligibleForRetry += topic // 把该主题加到待重试主题列表中 后续重试
        }
      }

      // Add topic to the eligible set if it is eligible for deletion.
      if (isTopicEligibleForDeletion(topic)) { // 如果该主题能够被删除
    ...
    if (topicsEligibleForDeletion.nonEmpty) { // 对待删除主题列表中的主题执行删除操作
